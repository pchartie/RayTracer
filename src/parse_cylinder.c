...
static void	parse_cylinder(t_infos *i, char **split, int i_obj)
{
	if (split[0] != 0)
	{
		if (ft_strcmp(split[0], "\tPOS") == 0
			...
			&& split[1] != 0 && split[2] != 0 && split[3] != 0)
			get_dir(i, split, i_obj, CYLINDER);
			...
			&& split[1] != 0)
		{
			...
			get_bonus_value(i, split, i_obj, CYLINDER);
		i->obj_type[i_obj] = CYLINDER;
		i->obj_id[i_obj] = (cl_uchar)i_obj;
	}
}

void		get_cylinder_value(t_infos *i, int i_obj, int fd, char *line)
{
	char **split;
